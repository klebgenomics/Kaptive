...
class Serotyper:
    ...
    def __call__(self, genome: GenomeAssembly | str | Path) -> SerotypingResult | None:
        ...
        best_gene_aln_scores = (
            best_gene_alns.query_weighted_scores
...
class KaptiveRow(ReportRow):
    ...
    def from_result(cls, result: SerotypingResult):
        ...
        comp = result.best_locus_completeness
        expected_total = int(round(n_exp_in / comp)) if comp > 0 else 0
        exp_in_str = (
            b"%d / %d (%.2f%%)" % (n_exp_in, expected_total, comp * 100)
            if expected_total
            else b"0 / 0 (0.00%)"
        )
        ...
        out_comp = (n_exp_out / expected_total * 100) if expected_total else 0.0
        exp_out_str = (
    ...
    def from_result(cls, result: SerotypingResult):
        ...
        if result.problems:
            ...
            if SerotypingProblem.UNEXPECTED_GENES in result.problems:
                detail_parts.append(b"uexpected gene(s) in locus")
