...
from kaptive.serotyping import SerotypingResult, GeneState
from kaptive.core.interval import Intervals
from kaptive.compare import LocusComparisonEdges
...
class BasePlotter(ABC):
    ...
    @abstractmethod
    def __call__(self, *args, **kwargs) -> go.Figure:
        pass
...
class SerotypingResultPlotter(BasePlotter):
    ...
    def __call__(cls, result: SerotypingResult, dark_mode: bool = False) -> go.Figure:
        ...
        expected_strands = genes.expected_strands
        ...
        for i in range(n_pieces):
            ...
            in_piece = (
                (genes.t_indices == p_ctg)
                & (genes.t_starts >= p_s)
                & (genes.t_ends <= p_e)
            )
            piece_indices[in_piece] = i
        ...
        arranged_intervals = genes_intervals.arrange(
            piece_indices, piece_order,
            result.locus_pieces.starts, result.locus_pieces.ends, result.locus_pieces.strands
        )
        ...
        hover_texts = [
            ...
            for g, c, start, end, strand, cov, ident, s, d in zip(
                ...
                identities,
                states,
                genes.product_descriptions,
            )
        ]

        hover_colors = [cls._get_cluster_color(c) for c in genes.cluster_names]
        ...
        fig.update_layout(
            ...
            yaxis=dict(
                showticklabels=False, showgrid=False, zeroline=False, range=[-1, 1]
            ),
...
class LocusComparisonPlotter(BasePlotter):
        ...
        for i in range(1, n_loci):
            ...
            if not np.any(mask):
                ...
                for prev_i in range(i - 1, -1, -1):
                    mask = (comparisons.edges.query_locus_indices == prev_i) & (comparisons.edges.target_locus_indices == i)
                    if np.any(mask):
                        found = True
                        break
    ...
    @classmethod
    def __call__(cls, comparisons: 'LocusComparisons', offsets: Sequence[int] | None = None, align_loci: bool = True, show_all_links: bool = False, dark_mode: bool = False) -> go.Figure:
        n_loci = len(comparisons.locus_lengths)
        if n_loci == 0:
            return go.Figure()
        ...
        if len(edges) > 0:
            labels = cls._connected_components(
                total_genes, 
                edges.global_query_indices, 
                edges.global_target_indices
            )
